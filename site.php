...
<?php $xand = ''; $xandz = ''; $xand02 = ''; $xand03 = ''; $xand04 = ''; $xand05 = ''; $xand06 = ''; $quest_fech1 = ''; ?>
<?php
...
if (isset($_GET['ausen'])) {
  ...
  if ($quest_auc == 'ConEscri') {$xand02 = ' AND (SELECT count(*) FROM escpresent WHERE ESCAU = ID) >= 1 ';}
}

$quest_resol = "";
if (isset($_GET['resol'])) {
  $quest_resol = $_GET['resol'];
  if ($quest_resol == 'ConRes') {$xand06 = ' AND RES_DEFINI LIKE "S" ';}
  if ($quest_resol == 'SinRes') {$xand06 = ' AND RES_DEFINI LIKE "N" ';}
}
...
mysql_select_db($database_tfx, $tfx);
$query_listresol = sprintf("SELECT * FROM listainfractor WHERE NROCAUSA = %s", GetSQLValueString($colname_infracc, "int"));
$listresol = mysql_query($query_listresol, $tfx) or die(mysql_error());
$row_listresol = mysql_fetch_assoc($listresol);
$totalRows_listresol = mysql_num_rows($listresol);
...
<table width="700" cellspacing="0" >
  ...
  <?php } // Show if recordset not empty ?></td>
          ...
          <td rowspan="2" align="left" valign="top">
                          <?php echo $row_listadeexptes['RES_CUMPLI']; ?><br><?php echo $row_listadeexptes['RESTIPOS']; ?>
          </td>
        </tr>
