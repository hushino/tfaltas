...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "formulario")) {
  $insertSQL = sprintf("UPDATE escmodelos SET ESCAU = %s, ESCNOM = %s, CONTEN = %s, CONTEN2FORMT = %s, CONTEN2 = %s WHERE IDESC =%s",
...
<head>
<meta charset="utf-8">
<title>Tribunal de Faltas - Escritos - <?php echo $row_escrito['ESCNOM']; ?></title>
...
<table style="width:80%">
  <tr>
    <th><p id="demo"></p></th>
    <th><h4>ESCRITO: <?php echo $row_escrito['ESCNOM']; ?></h4></th>

  </tr>
  <tr>
    <td style="width: 20%; vertical-align: text-top;"><div><?php do { ?><div ondragstart="dragStart(event)" draggable="true" id="<?php echo $row_comodin['COMODIN']; ?>" style="align:right; background-color:lightblue; margin: 2px; padding-left: 5px; border-style: dotted; border-width: 1px; width: auto;" title="<?php echo $row_comodin['COMODIN']; ?>"><?php echo $row_comodin['DESCRIPCION']; ?></div><?php } while ($row_comodin = mysql_fetch_assoc($comodin)); ?></div></td>
    <td style="width: 80%; vertical-align: text-top;"><form name="formulario" id="formulario" action="<?php echo $editFormAction; ?>" method="POST">
Nombre del Escrito: <input type="text" name="escnombre" id="escnombre" placeholder="Nombre del Escrito" value="<?php echo $row_escrito['ESCNOM']; ?>" style="width: 350px;" required autofocus>
...
<textarea name="area2" style="width: 600px;" ondrop="drop(event)" ondragover="allowDrop(event)">
<?php echo $row_escrito['CONTEN']; ?>
</textarea>
     </div>
     <div class="TabbedPanelsContent">
          Margen:
          <input type="radio" name="margen" value="exa3" <?php echo ($row_escrito['CONTEN2FORMT']=='exa3')?'checked':'' ; ?> /> Izquierdo
          <input type="radio" name="margen" value="exa2" <?php echo ($row_escrito['CONTEN2FORMT']=='exa2')?'checked':'' ; ?>/> Derecho<br>
